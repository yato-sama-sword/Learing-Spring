...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
    ...
    protected void doLoadBeanDefinitions(InputStream inputStream) {
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            if (childNodes.item(i) instanceof Element && BEAN_ELEMENT.equals(((Element) childNodes.item(i)).getNodeName())) {
                //解析bean标签
                ...
                for (int j = 0; j < bean.getChildNodes().getLength(); j++) {
                    if (bean.getChildNodes().item(j) instanceof Element && PROPERTY_ELEMENT.equals(((Element) bean.getChildNodes().item(j)).getNodeName())) {
                        //解析property标签
